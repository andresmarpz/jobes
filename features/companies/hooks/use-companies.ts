...
export function useCompanies(initialSortConfig?: SortConfig) {
  ...
  const loadCompanies = useCallback(async () => {
    ...
    const result = await Effect.runPromise(
      Effect.either(CompanyService.getCompanies)
    );
    if (result._tag === "Left") {
      setState((prev) => ({
        ...prev,
  ...
  const createCompany = useCallback(
    async (input: CreateCompanyInput) => {
      const result = await Effect.runPromise(
        Effect.either(CompanyService.createCompany(input))
      );
      if (result._tag === "Left") {
  ...
  const deleteCompany = useCallback(
    async (id: string) => {
      const result = await Effect.runPromise(
        Effect.either(CompanyService.deleteCompany(id))
      );
      if (result._tag === "Left") {
        throw new Error(result.left.message);
      }
...
export function useCompany(id: string) {
  ...
  const loadCompany = useCallback(async () => {
    ...
    setError(null);
    const result = await Effect.runPromise(
      Effect.either(CompanyService.getCompany(id))
    );
    if (result._tag === "Left") {
  ...
  const addContact = useCallback(
    async (input: CreateContactInput) => {
      const result = await Effect.runPromise(
        Effect.either(CompanyService.addContact(id, input))
      );
      if (result._tag === "Left") {
        throw new Error(result.left.message);
      }
